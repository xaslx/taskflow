...
from src.schemas.user import UserCreateSchema, UserLoginSchema, UserOut, UserUpdateSchema
...
class TestRegisterUserUseCase:
    ...
    async def test_register_user_success(self, register_usecase: RegisterUserUseCase, user_repository: BaseUserRepository):
    ...
    async def test_register_user_already_exists(self, register_usecase: RegisterUserUseCase):
        new_user = UserCreateSchema(email='test@test.com', password='test123456')
...
class TestLoginUserUseCase:
    ...
    async def test_login_user_success(self, login_use_case: LoginUserUseCase):
        credentials: UserLoginSchema = UserLoginSchema(email='testuser@gmail.com', password='testtest123')
    ...
    async def test_login_user_fail(self, login_use_case: LoginUserUseCase):
        with pytest.raises(IncorrectEmailOrPasswordException):
            credentials: UserLoginSchema = UserLoginSchema(email='testuser@gmail.com', password='testtest100')
...
class TestUpdateUserUseCase:
    ...
    async def test_update_email(
        ...
        user_out: UserOut = await update_user_use_case.execute(updated_user=update_data, user=test_user)

        assert user_out.email != old_email

        updated_user_model: UserModel | None = await user_repository.get_by_email(email='new_email@gmail.com')
    ...
    async def test_update_password(
        ...
        updated_user_model: UserModel | None = await user_repository.get_by_email(email=test_user.email)
