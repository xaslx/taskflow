...
from src.exceptions.user import ForbiddenException, ManagerNotInTeamException, UnauthorizedException
...
from src.repositories.task import BaseTaskCommentRepository, SQLAlchemyTaskCommentRepository
from src.repositories.evaluation import BaseEvaluationRepository, SQLAlchemyEvaluationRepository
...
class AppProvider(Provider):
    ...
    @provide(scope=Scope.REQUEST)
    def get_task_comment_repository(self, session: AsyncSession) -> BaseTaskCommentRepository:
        return SQLAlchemyTaskCommentRepository(_session=session)
    
    @provide(scope=Scope.REQUEST)
    def get_evaluation_repository(self, session: AsyncSession) -> BaseEvaluationRepository:
    ...
    @provide(scope=Scope.REQUEST)
    def get_refresh_token_use_case(self, jwt_service: JWTService) -> RefreshTokenUseCase:
    ...
    @provide(scope=Scope.REQUEST)
    def get_delete_user_use_case(self, user_repository: BaseUserRepository) -> DeleteUserUseCase:
        return DeleteUserUseCase(_user_repository=user_repository)
    


    @provide(scope=Scope.REQUEST)
    def get_delete_user_by_admin_use_case(self, user_repository: BaseUserRepository) -> DeleteUserByAdminUseCase:

        return DeleteUserByAdminUseCase(_user_repository=user_repository)
    

    @provide(scope=Scope.REQUEST)
    def get_create_team_use_case(self, team_repository: BaseTeamRepository) -> CreateTeamUseCase:
