...
class CreateMeetingUseCase:
    ...
    async def execute(
        ...
        for user_id in meeting_data.participant_ids + [organizer_id]:
            ...
            conflicting_meetings: MeetingModel | None = (
                await self._meeting_repository.get_user_meetings_in_time_range(
                    user_id, meeting_data.start_time, meeting_data.end_time
                )
            )
        ...
        meeting: MeetingModel = await self._meeting_repository.add(
