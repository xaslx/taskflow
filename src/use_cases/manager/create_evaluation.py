...
class CreateEvaluationUseCase:
    ...
    async def execute(self, evaluation: EvaluationCreate, evaluator_id: int, team_id: int) -> EvaluationOut:
 
        task: TaskModel | None = await self._task_repository.get_by_id(id=evaluation.task_id)
        ...
        existing_evaluation: EvaluationModel | None = await self._evaluation_repository.get_by_task_id(task.id)
