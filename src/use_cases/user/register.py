...
class RegisterUserUseCase:
    ...
    async def execute(self, new_user: UserCreateSchema) -> UserOut:
        ...
        user: UserModel | None = await self._user_repository.get_by_email(email=new_user.email)

        if user:
            raise UserAlreadyExists()
        
        hashed_password: str = self._hash_service.get_password_hash(password=new_user.password)

        user: UserModel = await self._user_repository.add(user=new_user, hashed_password=hashed_password)
